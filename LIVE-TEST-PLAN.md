...
> ⚠️ **BLOCKED by a known architectural gap (found in Stage 0.3, before Ian).** Cross-party issue/PR replication does **not** work yet. Each forge constructs its issues/PR (and multi-writer refs) Autobase with `bootstrap=null` and the autobase key is **never published in the manifest `cores` record** (only `refs/objects/objectIndex/meta/metaKeys` are). So every forge has an **isolated, local-only** issues/PR ledger — Bob's issue never reaches Alice and vice-versa. Proven empirically: a 2-forge swarm probe shows `Bob→sees→Alice: NO`, `Alice→sees→Bob: NO` (`Alice listIssues=1` = only her own). The git-data path (push/clone/replicate) is unaffected and fully proven cross-party. **Stages 1–3 do not depend on this; Stage 4's forge-loop does.** Fix design: publish the issues/PR Autobase key in the manifest; second forge opens with that bootstrap key; contributor write-access via the already-designed-but-unwired `meta.bootstrap.{owners,writers}` + Autobase `addWriter` handshake (`makeApply` authorization already exists). Non-trivial; design decision pending.

Pipe proven → run the full forge loop on the real repo:
- **Prereq:** cross-party issue/PR Autobase sharing implemented + dry-run-proven (the gap above).
...
- [ ] Stage 1 — **ready to schedule with Ian** (on the real public DHT — the authoritative cross-process validation). The git-data path (push/clone/replicate of the real Opengit repo) is proven in-harness cross-party; the single remaining unproven variable for Stage 1 is the subprocess-over-real-DHT discovery hop, which is precisely Stage 1's job. The signed issue/PR *primitive* (open/sign/readback) is proven; cross-party issue/PR *replication* is **not** (see Stage 4 ⚠️) — that gap blocks Stage 4 only, not Stages 1–3.
- [ ] **Stage 4 prerequisite — cross-party issue/PR Autobase sharing.** Found in 0.3: issues/PR/multi-writer-refs Autobases are forge-local silos (`bootstrap=null`, key unpublished). Design + implement + dry-run-prove before the Stage-4 portion of the live session.
