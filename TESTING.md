...
the live-test plan (stages + failure playbook). In-harness rehearsal that this
...
- Stage 3 (Ian's turf): point both sides at a HiveRelay bootstrap and/or use the
  ...
  ```
  Re-run Stage 1 with that set. See the live-test plan §Stage 3 + failure table.
...
5. Everything else → the live-test plan failure-playbook table.
